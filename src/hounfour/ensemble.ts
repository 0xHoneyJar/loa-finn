  ...
  try {
    ...
    for (let i = 0; i < successful.length; i++) {
      const completionResult: CompletionResult = {
        ...
        metadata: { model: successful[i].pool, latency_ms: 0, trace_id: "" },
      }
    ...
    const selected: CompletionResult = {
      ...
      tool_calls: null,
      usage: best.usage,
      metadata: { model: best.pool, latency_ms: 0, trace_id: "" },
    }
...
export async function consensusStreaming(
  ...
  try {
    ...
    const completionResults: CompletionResult[] = successful.map((r) => ({
      ...
      tool_calls: null,
      usage: r.usage,
      metadata: { model: r.pool, latency_ms: 0, trace_id: "" },
    }))
    ...
    if (parsedFields.length >= 2) {
      ...
      selected = {
        ...
        metadata: { model: `ensemble:consensus:${pools.join("+")}`, latency_ms: 0, trace_id: "" },
      }
    } else {
