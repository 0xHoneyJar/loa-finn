...
# cardIds whose Ability negates ALL ex/Mega-ex damage (Sylveon Safeguard / Crustle Mysterious Rock Inn).
# Co-design constant: the pilot knows its own wall (cardId is always available at runtime; ability text may not be).
_EX_IMMUNE_WALLS = {330, 345}
...
def _opp_max_dmg(state, horizon):
    ...
    if ocard is None:
        return (0, my_hp)
    # co-design: an ex-immune wall takes 0 from the opponent's ex/Mega-ex (Sylveon Safeguard / Crustle
    # Mysterious Rock Inn) -> an ex attacker is NOT a threat. Without this the pilot reads the ex's big
    # number, wastes heals, and mis-sequences vs a deck it actually hard-walls.
    if _g(ma, "id") in _EX_IMMUNE_WALLS and (getattr(ocard, "ex", False) or getattr(ocard, "megaEx", False)):
        return (0, my_hp)
...
def _main_score(state, opt):
    ...
    if t == OT.PLAY:
        ...
        if hmin is not None:                                 # recoils 70/turn; Cook/Jumbo offset it).
            ...
            return min(740 + dmg_on_me, 870)                 # stall-max: scale heal with damage -> chain Jumbo items
            #                                                  + beat draw(790) once hurt ~50+ (wall over-heal attrition)
