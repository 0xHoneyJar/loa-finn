// tests/finn/damp-modes.test.ts — dAMP Mode Override & Cache Test Suite (Sprint 8 Task 8.4)
...
describe("dAMP Mode Overrides (Sprint 8 Task 8.1)", () => {
  beforeEach(() => {
    clearArtifactCache()
    resetDAMPTablesCache()
  })
  ...
  it("brainstorm mode shifts creativity dials upward", () => {
    const fpDefault = deriveDAMP(MOCK_SNAPSHOT, "default")
    const fpBrainstorm = deriveDAMP(MOCK_SNAPSHOT, "brainstorm")
  ...
  it("critique mode shifts epistemic dials upward", () => {
    const fpDefault = deriveDAMP(MOCK_SNAPSHOT, "default")
    const fpCritique = deriveDAMP(MOCK_SNAPSHOT, "critique")
  ...
  it("execute mode shifts convergence dials upward", () => {
    const fpDefault = deriveDAMP(MOCK_SNAPSHOT, "default")
    const fpExecute = deriveDAMP(MOCK_SNAPSHOT, "execute")
  ...
  it("each mode produces measurably different dial values from default", () => {
    ...
    for (const mode of modes) {
      const fp = deriveDAMP(MOCK_SNAPSHOT, mode)
      let differCount = 0
  ...
  it("brainstorm mode contains required dial offsets", () => {
    ...
    expect(brainstorm["cr_divergent_thinking" as DAMPDialId]).toBe(0.3)
    expect(brainstorm["cr_originality_drive" as DAMPDialId]).toBe(0.25)
    expect(brainstorm["ep_speculation_comfort" as DAMPDialId]).toBe(0.25)
  })

  it("critique mode contains required dial offsets", () => {
    const tables = getDAMPTables()
    const critique = tables.mode_deltas["critique"]
    expect(critique["ep_evidence_threshold" as DAMPDialId]).toBe(0.3)
    expect(critique["cv_completeness_need" as DAMPDialId]).toBe(0.2)
  })

  it("execute mode contains required dial offsets", () => {
    const tables = getDAMPTables()
    const execute = tables.mode_deltas["execute"]
    expect(execute["cv_closure_drive" as DAMPDialId]).toBe(0.3)
    expect(execute["cv_feasibility_weight" as DAMPDialId]).toBe(0.3)
    expect(execute["cv_scope_discipline" as DAMPDialId]).toBe(0.3)
  })
})
...
// ---------------------------------------------------------------------------
// dAMP Mode Cache Tests (Sprint 8 Task 8.3)
// ---------------------------------------------------------------------------

describe("dAMP Mode Cache (Sprint 8 Task 8.3)", () => {
  ...
  it("cache key includes mode", async () => {
    const fingerprint = deriveDAMP(MOCK_SNAPSHOT, "brainstorm")
    await service.setDAMPCached("bears", "42", "brainstorm", fingerprint)

    expect(redis._store.has("damp:cache:bears:42:brainstorm")).toBe(true)
    expect(redis._store.has("damp:cache:bears:42:default")).toBe(false)
  })
  ...
  it("different modes are cached independently", async () => {
    ...
    await service.setDAMPCached("bears", "42", "brainstorm", fpBrainstorm)
    await service.setDAMPCached("bears", "42", "critique", fpCritique)

    const cachedBrainstorm = await service.getDAMPCached("bears", "42", "brainstorm")
    const cachedCritique = await service.getDAMPCached("bears", "42", "critique")
  ...
  it("cache hit returns stored fingerprint", async () => {
    const fingerprint = deriveDAMP(MOCK_SNAPSHOT, "execute")
    await service.setDAMPCached("bears", "42", "execute", fingerprint)

    const cached = await service.getDAMPCached("bears", "42", "execute")
  ...
  it("cache miss returns null", async () => {
    const cached = await service.getDAMPCached("bears", "42", "default")
    expect(cached).toBeNull()
  })

  it("cache stores with 1h TTL", async () => {
    const fingerprint = deriveDAMP(MOCK_SNAPSHOT, "default")
    await service.setDAMPCached("bears", "42", "default", fingerprint)

    const ttl = redis._ttls.get("damp:cache:bears:42:default")
    expect(ttl).toBe(3600)
  })

  it("invalidateDAMPCache deletes all 4 mode variants", async () => {
    // Pre-populate cache for all modes
    for (const mode of ["default", "brainstorm", "critique", "execute"] as AgentMode[]) {
      const fp = deriveDAMP(MOCK_SNAPSHOT, mode)
      await service.setDAMPCached("bears", "42", mode, fp)
    }

    // Verify all are cached
    expect(redis._store.has("damp:cache:bears:42:default")).toBe(true)
    expect(redis._store.has("damp:cache:bears:42:brainstorm")).toBe(true)
    expect(redis._store.has("damp:cache:bears:42:critique")).toBe(true)
    expect(redis._store.has("damp:cache:bears:42:execute")).toBe(true)

    // Invalidate
    await service.invalidateDAMPCache("bears", "42")

    // Verify all are gone
    expect(redis._store.has("damp:cache:bears:42:default")).toBe(false)
    expect(redis._store.has("damp:cache:bears:42:brainstorm")).toBe(false)
    expect(redis._store.has("damp:cache:bears:42:critique")).toBe(false)
    expect(redis._store.has("damp:cache:bears:42:execute")).toBe(false)
  })

  it("update() invalidates dAMP cache for the updated NFT", async () => {
    ...
    // Pre-populate dAMP cache
    const fp = deriveDAMP(MOCK_SNAPSHOT, "brainstorm")
    await service.setDAMPCached("bears", "42", "brainstorm", fp)
    expect(redis._store.has("damp:cache:bears:42:brainstorm")).toBe(true)

    // Update personality
    await service.update("bears", "42", { name: "UpdatedBear" })

    // dAMP cache should be invalidated
    expect(redis._store.has("damp:cache:bears:42:brainstorm")).toBe(false)
  })
