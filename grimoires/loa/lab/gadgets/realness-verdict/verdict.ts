// gadgets/realness-verdict/verdict.ts — GADGET #001 · the realness verdict computer.
//
// Spec sheet (Finn's shop tag):
...
//   WHY    the SETTLE step's verdict was ad-hoc inline `node` each run (SETTLE-001/2/3).
//          A reproducible pure function makes the verdict a gadget, not a judgment call.
//   IN     a daily series + top-1 concentration + (optional) pinned bars.
//   OUT    a verdict + every computed metric + which bar each leg hit.
//   DEPS   none. Pure. No network, no LLM. Settles are deterministic (epistemology §4).
//   STATUS GRADUATED 2026-06-14 (sprint:corpus-a T3) → src/research/realness-verdict.ts
//          is now the AUTHORITATIVE product copy (imported by src/corpus/settle.ts).
//          This file is the studio ORIGIN, kept for provenance — do not import it.
//
// This is the SETTLE math extracted, not new logic: it reproduces SETTLE-003's
// HELD[real] on the Kintara series and a THEATER verdict on an x402-shaped series
// (the discrimination acceptance test, made executable — see verdict.test.ts).
