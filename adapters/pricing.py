"""Integer micro-USD pricing — cherry-picked from loa_cheval/metering/pricing.py (SDD §4.5).

All prices in micro-USD per million tokens. 1 USD = 1,000,000 micro-USD.
No floating-point anywhere in the cost path.

Cross-language parity: golden vectors in packages/loa-hounfour/vectors/budget/
"""
...
# Default pricing table for common models (matches src/hounfour/pricing.ts)
DEFAULT_PRICING: list[PricingEntry] = [
    PricingEntry("qwen-local", "Qwen/Qwen2.5-7B-Instruct", 10_000, 10_000),
    PricingEntry("qwen-local", "Qwen/Qwen2.5-Coder-7B-Instruct", 10_000, 10_000),
    PricingEntry("openai", "gpt-4o", 2_500_000, 10_000_000),
    PricingEntry("openai", "o3", 10_000_000, 40_000_000, 40_000_000),
    PricingEntry("anthropic", "claude-opus-4-6", 15_000_000, 75_000_000, 75_000_000),
    PricingEntry("anthropic", "claude-sonnet-4-5-20250929", 3_000_000, 15_000_000, 15_000_000),
]


def find_default_pricing(provider: str, model: str) -> Optional[PricingEntry]:
    """Look up pricing from the built-in default table."""
